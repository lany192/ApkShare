...
public class AppAdapter extends RecyclerView.Adapter<AppAdapter.AppViewHolder> {
	...
	public AppViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
		View view = LayoutInflater.from(viewGroup.getContext()).inflate(
				R.layout.app_listview_item, viewGroup, false);
	...
	public void onBindViewHolder(AppViewHolder appViewHolder, final int i) {
		...
		appViewHolder.cardView.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				shareApkIntent.putExtra(Intent.EXTRA_STREAM,
						Uri.fromFile(new File(apps.get(i).getApkPath())));
				shareApkIntent
						.setType("application/vnd.android.package-archive");
				context.startActivity(Intent.createChooser(shareApkIntent,
						context.getString(R.string.share_apk_dialog_title)));
			}
		});
	}
	...
	private String getHumanReadableSize(long apkSize) {
		...
		if (apkSize < 1024) {
			humanReadableSize = String.format(
					context.getString(R.string.app_size_b), (double) apkSize);
